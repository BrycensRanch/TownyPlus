...
public class Gson {
  ...
  public JsonObject http(String urlString, String body, Boolean useCredentials)
    throws IOException {
    return http(urlString, body, "POST", useCredentials);
  }

  public JsonObject http(
    String urlString,
    String body,
    String method,
    ...
    con.setRequestProperty(
      "Authorization",
      TownyPlusMain.plugin.config.getString("restapi.apiKey")
    );
    ...
    try (
      ...
      if (isJSONValid(jsonResponseString) == false) throw new BindException(
        "The response from the api specified didn't respond with a valid JSON object. Got: " +
        jsonResponseString
      );
