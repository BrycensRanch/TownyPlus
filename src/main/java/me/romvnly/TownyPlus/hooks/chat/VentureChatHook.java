...
public class VentureChatHook implements ChatHook {
    ...
    public void onVentureChat(VentureChatEvent event) {
        ...
        if (chatChannel == null) {
            TownyPlusMain.plugin.logger.warn("VentureChatHook got a null chatChannel from the EventChatEvent. Ignoring event.");
            return;
        }
        ...
        TownyPlusMain.plugin.processChatMessage(player, message, chatChannel.getName(), this);
        return;
    ...
    public void broadcastMessageToChannel(String channel, Component component, Town town) {
        ...
        for (MineverseChatPlayer player : playersToNotify) {
            String playerMessage = (player.hasFilter() && chatChannel.isFiltered()) ? Format.FilterChat(legacy) : legacy;
