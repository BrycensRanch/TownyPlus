...
public class DumpInfo {
  ...
  public DumpInfo(boolean addLog) {
    ...
    try (
      ...
      String json = javaPropsMapper
        .readPropertiesAs(gitProp, JsonNode.class)
        .toPrettyString();
    ...
    try {
      ...
      File file = new File(
        DumpInfo.class.getProtectionDomain()
          .getCodeSource()
          .getLocation()
          .toURI()
      );
      ...
      TownyPlusMain
        .getInstance()
        .getLogger()
        .warning("Unable to get hash of JAR file");
      e.printStackTrace();
    }
  ...
  public static class LogsInfo {
    ...
    public LogsInfo() {
      try {
        ...
        Path latestLogFilePath = FileSystems
          .getDefault()
          .getPath("logs", "latest.log");
        fields.put(
          "content",
          FileUtils
            .readAllLines(latestLogFilePath)
            .collect(Collectors.joining("\n"))
        );

        JsonNode logData = TownyPlusMain.JSONMapper.readTree(
          WebUtils.postForm("https://api.mclo.gs/1/log", fields)
        );
