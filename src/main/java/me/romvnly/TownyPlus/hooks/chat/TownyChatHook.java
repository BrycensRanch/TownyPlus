...
public class TownyChatHook implements ChatHook {
  ...
  public void reload() {
    ...
    if (instance == null) {
      TownyPlusMain.plugin
        .getLogger()
        .warning("Could not automatically hook TownyChat channels");
      return;
    }
    ...
    hookedChannels.forEach(name -> {
      ...
      if (channel != null) {
        ...
        TownyPlusMain.plugin
          .getLogger()
          .info(
            "It appears as the town and nation channels do not exist on TownyChat, not hooking anything."
          );
      }
    });
    for (Channel channel : instance
      .getChannelsHandler()
      .getAllChannels()
      .values()) {
    ...
    if (!linkedChannels.isEmpty()) {
      TownyPlusMain.plugin
        .getLogger()
        .info(
          "Marked the following TownyChat channels as hooked: " +
          (String.join(", ", linkedChannels)) +
          ". Available channels: " +
      ...
      TownyPlusMain.plugin
        .getLogger()
        .info(
          "No TownyChat channels were marked as hooked. Available channels: " +
          String.join(", ", availableChannels)
        );
    }
  }
  ...
  public void broadcastMessageToChannel(
    ...
    String plainMessage = LangUtil.Message.CHAT_CHANNEL_MESSAGE
      .toString()
      .replace(
        "%channelcolor%",
        destinationChannel.getMessageColour() != null
      ...
      .replace(
        "%channelnickname%",
        destinationChannel.getChannelTag() != null
    ...
    for (Player player : PlayerUtil.getOnlinePlayers()) {
      if (destinationChannel.isPresent(player.getName())) {
        if (channel == "town") {
          ...
          if (
            residentTown != null &&
            residentTown.hasResident(player.getUniqueId())
          ) {
          ...
          if (
            residentNation != null &&
            residentNation.hasResident(player.getName().toString())
          ) {
            player.sendMessage(plainMessage);
          }
        }
    ...
    PlayerUtil.notifyPlayersOfMentions(
      player -> destinationChannel.isPresent(player.getName()),
      legacy
    );
  }
  ...
  private static Channel getChannelByCaseInsensitiveName(String name) {
    ...
    for (Channel townyChannel : instance
      .getChannelsHandler()
      .getAllChannels()
      .values()) if (
      townyChannel.getName().equalsIgnoreCase(name)
    ) return townyChannel;
    return null;
  }
