...
public class RestAPI {
    ...
    public void startServer(String host, Integer port) {
        ...
        server.before(ctx -> {
            ...
            if (requestJSONBody == null && (ctx.method().toString() != "GET" && ctx.body().length() == 0)) {
                ctx.status(400);
                ctx.json(new Gson().toJson(new StandardResponse(StatusResponse.ERROR, "Couldn't parse your request body as JSON.")));
            }
